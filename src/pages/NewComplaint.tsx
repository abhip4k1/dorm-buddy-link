...
const NewComplaint = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const generatedId = `PU-HST-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 9999)).padStart(4, '0')}`;
      ...
      <Layout title="Complaint Filed" showBack>
        ...
        >
          ...
          <div className="bg-card p-5 rounded-2xl shadow-card border border-border/50 mb-6 w-full max-w-xs">
            ...
            <p className="text-lg font-bold text-foreground mt-1">#{complaintId}</p>
          </div>
  ...
  return (
    <Layout title="File a Complaint" showBack>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <Label className="text-foreground font-semibold text-sm mb-3 block">Select Category *</Label>
              ...
              return (
                <motion.button
                  ...
                  className={`p-4 rounded-2xl border-2 transition-all duration-200 ${
                    isSelected ? "border-primary bg-primary/5 shadow-md shadow-primary/10" : "border-border/50 bg-card hover:border-primary/30"
                  }`}
                >
                  <div className={`w-10 h-10 rounded-xl ${category.color} flex items-center justify-center mx-auto mb-2`}>
                    <Icon className="w-5 h-5 text-white" />
                  </div>
                  <p className={`text-xs font-semibold ${isSelected ? "text-primary" : "text-foreground"}`}>{category.label}</p>
                </motion.button>
              );
            })}
          </div>
        ...
        <div>
          <Label htmlFor="description" className="text-foreground font-semibold text-sm mb-3 block">Describe the Issue *</Label>
          <Textarea
            id="description"
          ...
          />
          <p className="text-[11px] text-muted-foreground mt-2 font-medium">Be specific about the location and nature of the problem</p>
        </div>

        <div>
          <Label className="text-foreground font-semibold text-sm mb-3 block">Upload Evidence (Optional)</Label>
          <button type="button" onClick={handleFileUpload} className="w-full p-6 border-2 border-dashed border-border/60 rounded-2xl hover:border-primary/40 transition-colors bg-secondary/30">
            <Upload className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
            <p className="text-sm text-muted-foreground font-medium">Tap to upload photos or videos</p>
          </button>
            ...
            <div className="mt-3 space-y-2">
              {uploadedFiles.map((file, index) => (
                <div key={index} className="flex items-center justify-between bg-card p-3 rounded-xl border border-border/50">
                  ...
                  <button type="button" onClick={() => removeFile(index)} className="p-1 hover:bg-destructive/10 rounded-full transition-colors">
        ...
        <Button type="submit" size="xl" className="w-full rounded-xl gradient-primary shadow-glow" disabled={!selectedCategory || !description.trim() || isSubmitting}>
          {isSubmitting ? (
