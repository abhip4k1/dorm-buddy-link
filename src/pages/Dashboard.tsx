...
const Dashboard = () => {
  ...
  return (
    <Layout>
      ...
      <Link 
        to="/emergency"
        className="fixed bottom-28 right-5 md:bottom-8 md:right-8 z-40"
      ...
      {/* Desktop top spacer for fixed nav */}
      ...
      >
        <div className="relative gradient-hero rounded-3xl p-5 md:p-8 lg:p-10 overflow-hidden shadow-lg">
          ...
          <motion.div 
            ...
            className="absolute top-4 right-8 text-3xl md:text-5xl"
          >
            🏠
          </motion.div>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3.5 md:gap-5 mb-4">
              ...
              <div className="flex-1">
                <h1 className="text-lg md:text-2xl lg:text-3xl font-bold text-white">Hello, Abhi! 👋</h1>
                <p className="text-xs md:text-sm text-white/60 font-medium">Azad Bhavan B • Room 411</p>
              </div>
            </div>
            ...
            <div className="flex gap-2 md:gap-3">
              <div className="flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/5">
                ...
                <span className="text-[11px] md:text-sm font-semibold text-white/80">B.Tech CSE • 3rd Year</span>
              </div>
      ...
      {/* Desktop: 2-column layout for main content */}
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          ...
          <section>
            <div className="flex items-center justify-between mb-3 md:mb-4">
              ...
              <span className="text-[10px] md:text-xs font-semibold text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">6 services</span>
            </div>
            <div className="grid grid-cols-3 md:grid-cols-6 gap-3 md:gap-4">
              {quickActions.map((action, idx) => (
                ...
                >
                  <Link
                    to={action.to}
                    className="block p-4 md:p-5 rounded-2xl bg-card border border-border/60 shadow-card hover:shadow-card-hover active:scale-[0.96] transition-all duration-200 text-center group"
                  >
                    <motion.div 
                      whileHover={{ rotate: [0, -10, 10, 0] }}
                      transition={{ duration: 0.4 }}
                      className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl ${action.gradient} flex items-center justify-center mx-auto mb-2.5 shadow-sm group-hover:shadow-md transition-shadow`}
                    >
            ...
            <Link to="/announcements" className="block group">
              <div className="bg-card border border-border/60 rounded-2xl p-4 md:p-5 flex items-center gap-3 md:gap-4 shadow-card hover:shadow-card-hover transition-all relative overflow-hidden">
                ...
                <motion.div 
                  ...
                  className="w-11 h-11 md:w-14 md:h-14 rounded-2xl gradient-warning flex items-center justify-center flex-shrink-0 shadow-sm relative z-10"
                >
                  <Bell className="w-5 h-5 md:w-6 md:h-6 text-white" />
                </motion.div>
                <div className="flex-1 min-w-0 relative z-10">
                  <div className="flex items-center gap-2">
                    <p className="text-sm md:text-base font-bold text-foreground">3 New Announcements</p>
                    <span className="w-2 h-2 rounded-full bg-destructive animate-pulse" />
                  </div>
                  <p className="text-xs md:text-sm text-muted-foreground font-medium truncate mt-0.5">Water supply maintenance tomorrow...</p>
                </div>
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <section>
            <div className="flex items-center justify-between mb-3 md:mb-4">
              ...
              <Link to="/announcements" className="text-xs md:text-sm text-primary font-semibold hover:underline underline-offset-2">
                See All
              </Link>
            </div>
            <div className="bg-card rounded-2xl border border-border/60 shadow-card overflow-hidden">
              {recentUpdates.map((update, index) => (
                <motion.div
                  ...
                  className={`flex items-center gap-3 p-4 md:p-5 ${index < recentUpdates.length - 1 ? "border-b border-border/40" : ""}`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[10px] md:text-xs font-bold px-2 py-0.5 rounded-full ${tagColors[update.tag]}`}>
                        {update.tag}
                      </span>
                      <span className="text-[11px] md:text-xs text-muted-foreground">{update.time}</span>
                    </div>
                  </div>
                </motion.div>
              ))}
